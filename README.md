...
nix build .#libutil .#fileCmds    # libutil.dylib; ls, cp, mv, chmod, ... from file_cmds (stage 6)
...
| `libutil` / `fileCmds` | Stage 6: `/usr/lib/libutil.dylib` (+ `libutil1.0.dylib`, man pages in `/usr/local/share/man/man3`) and twelve `file_cmds` tools with their man pages: `/bin/{chmod,cp,ln,ls,mkdir,mv}`, `/usr/bin/{chflags,du,stat,truncate,xattr}` and `/usr/sbin/chown`. `ls` links libutil (`humanize_number`) and libncurses (termcap, for `-G`), `du` and `truncate` link libutil, as Apple's do; the rest link `libSystem` only. libutil is built without `tzlink.c` (needs `<xpc/xpc.h>`) and `wipefs.cpp`/`ExtentManager.cpp` (need IOKit headers), and exports everything else in `libutil.exports`. Declared absent: user/group name and membership lookups (`system_info`), and `fcopyfile`/`copyfile_state_*` (`copyfile`) -- so `cp` cannot copy a regular file's contents yet, and `mv` only fails across file systems. The hardlinked names Apple's aggregates add (`chgrp`, `link`, `unlink`, `readlink`, ...) are separate targets and not built. |
| `rootfs` | 16 dylibs and 24 executables assembled at real paths with whole-tree checks (load commands resolve, no undeclared undefined symbols). Nothing runs yet - no `/usr/lib/dyld`. |
...
No `/usr/lib/dyld` yet (`libmach_o.a` builds; dyld link not started). Userland is twelve `shell_cmds` tools, including `find`, `id` and `ash` (with libedit, libncurses and the terminfo database), and twelve `file_cmds` tools (`ls`, `cp`, `mv`, `chmod`, ..., with libutil); no `bash`, so no `/bin/sh`. No `launchd` - last open source was 2013 and depends on unreleased `libxpc`.
